...
const adminController = {
  ...
  adminUpdateTag: async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({
        message:
          "An error occurred while updating the tag. Please try again later.",
      });
    }
  },
  ...
  adminAddProduct: async (req, res) => {
    try {
      ...
      const imageUrl = productImageFile
        ? `/product/${productImageFile.filename}`
        : "";
      const pdfUrl = productPdfFile
        ? `/product/${productPdfFile.filename}`
        : "";
  ...
  adminAddSubAdmin: async (req, res) => {
    try {
      ...
      if (role === "superadmin") {
        ...
        return res
          .status(200)
          .json({ message: "Super Admin added successfully." });
        ...
        return res
          .status(200)
          .json({ message: "Sub Admin added successfully." });
      } else {
  ...
  adminEditSubAdmin: async (req, res) => {
    try {
      ...
      if (existingInAdmin) {
        return res
          .status(400)
          .json({ message: "Email already exists in Admin model." });
      }

      // Check for existing email in SubAdmin (excluding the current subadmin being edited)
      const existingInSubAdmin = await SubAdmin.findOne({
        email,
        _id: { $ne: id },
      });
      if (existingInSubAdmin) {
        return res
          .status(400)
          .json({ message: "Email already exists in SubAdmin model." });
      }
      ...
      res
        .status(200)
        .json({ message: "Sub-admin updated successfully.", data: updated });
    } catch (error) {
  ...
  GetUserQuery: async (req, res) => {
    ...
    try {
      ...
      const formattedUserQueries = allQueries.map((userqueries) => {
        return {
          ...userqueries._doc,
          formattedDate: moment(userqueries.createdAt).format(
            "DD/MM/YYYY hh:mmA"
          ), // e.g. 12/05/2025 02:30PM
        };
      });
